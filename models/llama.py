...
from transformers.models.llama.modeling_llama import (ACT2FN, Cache, DynamicCache, AttentionMaskConverter,
                                                      _prepare_4d_attention_mask, _prepare_4d_causal_attention_mask,
                                                      _prepare_4d_causal_attention_mask_for_sdpa,
                                                      BaseModelOutputWithPast, CausalLMOutputWithPast,
                                                      SequenceClassifierOutputWithPast, PreTrainedModel,
                                                      ALL_LAYERNORM_LAYERS, is_torch_greater_or_equal_than_1_13,
                                                      add_start_docstrings,
                                                      add_start_docstrings_to_model_forward, is_flash_attn_2_available,
                                                      is_flash_attn_greater_or_equal_2_10, logging,
                                                      replace_return_docstrings, is_torch_fx_available, LlamaConfig)
...
def _make_causal_mask(input_ids_shape: torch.Size, dtype: torch.dtype, device: torch.device,
    ...
    return AttentionMaskConverter._make_causal_mask(input_ids_shape = input_ids_shape, dtype = dtype, device = device,
...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            gate_proj = torch.cat([F.linear(x, gate_proj_slices[i]) for i in range(self.config.pretraining_tp)],
                                  dim = -1)
...
class LlamaAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor] = None,
                ...
                rpi_attn_prob: Optional[torch.FloatTensor] = None, **kwargs, ) -> Tuple[
...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.LongTensor] = None,
                position_ids: Optional[torch.LongTensor] = None, past_key_value: Optional[Cache] = None,
    ...
    def _flash_attention_forward(self, query_states, key_states, value_states, attention_mask, query_length,
        ...
        if attention_mask is not None:
            ...
            attn_output_unpad = flash_attn_varlen_func(query_states, key_states, value_states,
                                                       cu_seqlens_q = cu_seqlens_q, cu_seqlens_k = cu_seqlens_k,
                                                       max_seqlen_q = max_seqlen_in_batch_q,
                                                       max_seqlen_k = max_seqlen_in_batch_k, dropout_p = dropout,
                                                       softmax_scale = softmax_scale, causal = causal, )

            attn_output = pad_input(attn_output_unpad, indices_q, batch_size, query_length)
        else:
            attn_output = flash_attn_func(query_states, key_states, value_states, dropout,
                                          softmax_scale = softmax_scale, causal = causal)
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor] = None,
        ...
        if output_attentions:
            ...
            return super().forward(hidden_states = hidden_states, attention_mask = attention_mask,
...
LLAMA_ATTENTION_CLASSES = {"eager": LlamaAttention, "flash_attention_2": LlamaFlashAttention2,
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor] = None,
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(hidden_states = hidden_states,
                                                                             ...
                                                                             use_cache = use_cache,
                                                                             rpi_attn_prob = rpi_attn_prob, **kwargs, )
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(self, input_ids: torch.LongTensor = None, attention_mask: Optional[torch.Tensor] = None,
        ...
        if self._use_flash_attention_2:
            ...
            attention_mask = _prepare_4d_causal_attention_mask_for_sdpa(attention_mask, (batch_size, seq_length),
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if i == len(self.layers) - 1 and rpi_attn_prob is not None:
                ...
                if self.gradient_checkpointing and self.training:
                    layer_outputs = self._gradient_checkpointing_func(decoder_layer.__call__, hidden_states,
                                                                      ...
                                                                      output_attentions, use_cache, rpi_attn_prob = rpi_attn_prob)
                else:
                    layer_outputs = decoder_layer(hidden_states, attention_mask = attention_mask,
                                                  ...
                                                  output_attentions = output_attentions, use_cache = use_cache, rpi_attn_prob = rpi_attn_prob)
            else:
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(self, input_ids: torch.LongTensor = None, attention_mask: Optional[torch.Tensor] = None,
                ...
                rpi_attn_prob: Optional[torch.FloatTensor] = None) -> Union[Tuple, CausalLMOutputWithPast]:
        r"""
        Args:
...
class LlamaForSequenceClassification(LlamaPreTrainedModel):
    ...
    def forward(self, input_ids: torch.LongTensor = None, attention_mask: Optional[torch.Tensor] = None,
        ...
        transformer_outputs = self.model(input_ids, attention_mask = attention_mask, position_ids = position_ids,
